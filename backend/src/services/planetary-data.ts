...
export interface PlanetaryData {
  ...
  diameter: number; // km
  mass: string | number; // kg (scientific notation or number)
  orbitalPeriod: number; // days
  avgDistance: number; // AU (Astronomical Units)
  composition: string[];
  atmosphereComposition?: string[];
  temperature: { min: number; avg: number; max: number }; // Kelvin
  moons?: string[];
...
class PlanetaryDataService {
  private nasaFactSheetBase = 'https://nssdc.gsfc.nasa.gov/planetary/fact_sheet';
  private jplSbdbBase = 'https://ssd-api.jpl.nasa.gov/sbdb';
  private horizonsApi = 'https://ssd.jpl.nasa.gov/api/horizons.api';

  /**
   * Fetch planetary data from NASA Fact Sheets (public domain)
   * Parser for: https://nssdc.gsfc.nasa.gov/planetary/fact_sheet/
   */
  async fetchNASAFactSheets(): Promise<Record<string, PlanetaryData>> {
    try {
      ...
      console.log(
        'NASA Fact Sheets available at:',
        this.nasaFactSheetBase
      );
  ...
  async fetchJPLSmallBodyData(bodyName: string): Promise<PlanetaryData | null> {
    try {
      const response = await axios.get(`${this.jplSbdbBase}.api`, {
        params: {
          des: bodyName, // Designation (name or number)
          fields: 'full',
        },
        timeout: 10000,
      });
      ...
      return {
        ...
        avgDistance: data.orbit?.a || 0, // semi-major axis in AU
    ...
    try {
      const response = await axios.get(this.horizonsApi, {
        params: {
          ...
          COMMAND: bodyId,
          EPHEM_TYPE: 'observer',
          CENTER: '500@399', // From Earth's center
          TLIST: 'now',
