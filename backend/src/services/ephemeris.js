...
import { validateBodyName } from '../utils/validation.js';
import { queueApiCall } from '../utils/rate-limiter.js';
/**
...
export default class EphemerisService {
    ...
    async getPlanetaryPosition(bodyName) {
        try {
            ...
            if (cached && Date.now() - cached.timestamp < this.cacheTimeout) {
                console.info('Cache hit for planet position', { body: bodyName });
                return cached.data;
            }
            // Validate input
            if (!validateBodyName(bodyName)) {
                console.warn('Invalid body name', { body: bodyName });
                return null;
            }
            console.info('Fetching ephemeris data', { body: bodyName });
            // Fetch from NASA JPL Horizons
            const position = await queueApiCall(() => this.fetchFromHorizons(bodyName), `Fetch ephemeris for ${bodyName}`);
            if (position) {
            ...
            console.error('Error fetching planetary position', {
                body: bodyName,
    ...
    async getAllPlanetPositions() {
        try {
            console.info('Fetching all planet positions');
            ...
            console.error('Error fetching all planet positions', {
                error: error instanceof Error ? error.message : String(error),
            });
            return [];
        }
    ...
    async fetchFromHorizons(bodyName) {
        try {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.debug('Using mock ephemeris data for development');
            ...
            if (!response.data) {
                console.warn('No data from Horizons API', { body: bodyName });
                return this.getMockPosition(bodyName);
            }
            ...
            console.warn('Failed to fetch from Horizons API, using mock data', {
                error: error instanceof Error ? error.message : String(error),
            });
            return this.getMockPosition(bodyName);
        }
    ...
    clearCache() {
        this.cache.clear();
        console.info('Ephemeris cache cleared');
    }
}
