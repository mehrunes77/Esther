...
import PlanetaryDataService from '../services/planetary-data.js';
...
router.get('/positions', async (req, res) => {
    try {
        console.info('Fetching planet positions');
        const positions = await ephemerisService.getAllPlanetPositions();
        if (!positions || positions.length === 0) {
            console.warn('No planet positions found, returning empty array');
        ...
        console.error('Error fetching planet positions', {
...
router.get('/:name', async (req, res) => {
    try {
        ...
        if (!position && !profile) {
            console.warn('Planet data not found', { body: name });
